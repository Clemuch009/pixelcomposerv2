...
function getAllowOrigin(request) {
  ...
  return allowed.includes(origin) ? origin : 'https://maepixel.pages.dev';
}

function json(data, status = 200, req = null) {
  const cors = req ? { 'Access-Control-Allow-Origin': getAllowOrigin(req), ...CORS_HEADERS } : { 'Access-Control-Allow-Origin': '*', ...CORS_HEADERS };
  return new Response(JSON.stringify(data), {
    status, headers: { ...cors, 'Content-Type': 'application/json' },
  });
}
function err(msg, status = 400, req = null) { return json({ ok: false, error: msg }, status, req); }
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'OPTIONS') {
      return new Response(null, { status: 204, headers: { 'Access-Control-Allow-Origin': allowOrigin, ...CORS_HEADERS } });
    }
    ...
    if (request.method === 'POST' && url.pathname === '/track') {
      ...
      } catch { return err('invalid JSON', 400, request); }
      ...
      if (/bot|crawl|spider|headless/i.test(ua)) return json({ ok: true, ignored: true }, 200, request);
    ...
    if (request.method === 'POST' && url.pathname === '/feedback') {
      let body; try { body = await request.json(); } catch { return err('invalid JSON', 400, request); }
      const rating  = parseInt(body.rating) || null;
      const message = (body.message || '').trim().slice(0, 2000);
      const uid     = (body.uid || 'unknown').slice(0, 64);
      if (!rating && !message) return err('empty submission', 400, request);
      const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
      const recent = await env.DB.prepare(
        `SELECT COUNT(*) as cnt FROM feedback WHERE ip = ? AND created_at > datetime('now', '-1 hour')`
      ).bind(ip).first();
      if (recent && recent.cnt >= 5) return err('too many submissions', 429, request);
      await env.DB.prepare(
    ...
    if (request.method === 'GET' && url.pathname === '/stats') {
      ...
      if (key !== env.ADMIN_KEY) return err('unauthorized', 401, request);
      const [
    ...
    if (request.method === 'GET' && url.pathname === '/feedback') {
      ...
      return json({ ok: true, total: rows.results.length, feedback: rows.results }, 200, request);
    }
    ...
    if (request.method === 'DELETE' && url.pathname.startsWith('/showcase/')) {
      ...
      if (!id) return err('invalid id', 400, request);
    ...
    if (request.method === 'PATCH' && url.pathname.startsWith('/showcase/')) {
      ...
      if (!fields.length) return err('nothing to update', 400, request);
      vals.push(id);
